...
namespace GUI
{
	...
	void LoadImages()
	{
		if (Logo == NULL)
		{
			// Create GitHub image loader instance for your repository
			static GitHubImageLoader imageLoader("Darguon", "HuhtalaHook", "main", "Assets/");

			// Try to load images from GitHub
			bool githubLoadSuccess = true;

			// Load each image from GitHub with error checking
			// Update these filenames to match the exact names in your GitHub repository
			githubLoadSuccess &= imageLoader.LoadTexture("logo.png", &Logo, &LogoW, &LogoH);
			githubLoadSuccess &= imageLoader.LoadTexture("visual_button.png", &MenuButton1, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("aimbot_button.png", &MenuButton2, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("misc_button.png", &MenuButton3, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("config_button.png", &MenuButton4, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("preview_img.png", &HitboxImage, &hitboxW, &hitboxH);
			githubLoadSuccess &= imageLoader.LoadTexture("visual_button_pressed.png", &MenuButton1Pressed, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("aimbot_button_pressed.png", &MenuButton2Pressed, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("misc_button_pressed.png", &MenuButton3Pressed, &buttonW, &buttonH);
			githubLoadSuccess &= imageLoader.LoadTexture("config_button_pressed.png", &MenuButton4Pressed, &buttonW, &buttonH);

			// If GitHub loading fails, fall back to local memory-based loading
			if (!githubLoadSuccess)
			{
				// Fallback to original memory-based loading
	...
	void DrawGui()
	{
		// BLACK & ORANGE THEME
		{
			...
			// Main colors - making background fully black
			...
			colors[ImGuiCol_Header] = ImVec4(0.90f, 0.45f, 0.00f, 0.80f);
			colors[ImGuiCol_HeaderHovered] = ImVec4(1.00f, 0.55f, 0.00f, 0.80f);
			colors[ImGuiCol_HeaderActive] = ImVec4(1.00f, 0.65f, 0.10f, 1.00f);

			// Buttons - vibrant orange
			colors[ImGuiCol_Button] = ImVec4(0.90f, 0.45f, 0.00f, 0.80f);
			colors[ImGuiCol_ButtonHovered] = ImVec4(1.00f, 0.55f, 0.00f, 0.80f);
			colors[ImGuiCol_ButtonActive] = ImVec4(1.00f, 0.65f, 0.10f, 1.00f);

			// Frame BG (checkboxes, radio buttons, etc.)
			colors[ImGuiCol_FrameBg] = ImVec4(0.15f, 0.15f, 0.15f, 0.80f);
			colors[ImGuiCol_FrameBgHovered] = ImVec4(0.90f, 0.45f, 0.00f, 0.40f);
			colors[ImGuiCol_FrameBgActive] = ImVec4(0.90f, 0.45f, 0.00f, 0.60f);

			// Tabs
			colors[ImGuiCol_Tab] = ImVec4(0.40f, 0.20f, 0.00f, 0.80f);
			colors[ImGuiCol_TabHovered] = ImVec4(0.90f, 0.45f, 0.00f, 0.80f);
			colors[ImGuiCol_TabActive] = ImVec4(0.90f, 0.45f, 0.00f, 1.00f);
			colors[ImGuiCol_TabUnfocused] = ImVec4(0.25f, 0.15f, 0.00f, 0.97f);
			colors[ImGuiCol_TabUnfocusedActive] = ImVec4(0.40f, 0.20f, 0.00f, 1.00f);
			...
			// Borders, separators
			colors[ImGuiCol_Border] = ImVec4(0.90f, 0.45f, 0.00f, 0.50f);
			colors[ImGuiCol_BorderShadow] = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
			colors[ImGuiCol_Separator] = ImVec4(0.90f, 0.45f, 0.00f, 0.50f);
			colors[ImGuiCol_SeparatorHovered] = ImVec4(1.00f, 0.55f, 0.00f, 0.78f);
			colors[ImGuiCol_SeparatorActive] = ImVec4(1.00f, 0.65f, 0.10f, 1.00f);
		...
		// Define BorderColor here - BEFORE LoadImages()
		ImColor BorderColor = ImColor(0.90f, 0.45f, 0.00f, 1.00f);
